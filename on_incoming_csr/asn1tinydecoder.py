...
def asn1_get_value_of_type(der,(ixs,ixf,ixl),asn1_type):
	asn1_type_table = {
		...
		'SEQUENCE':          0x10,	'SET':               0x11, # MODIFIED
...
# get tag number = get first 5 bits from identifier octet (8,7: Class; 6: P/C; 5-1: tag number)
def asn1_get_tag_number(der, (ixs,ixf,ixl)):
	return ord(der[ixs]) & 0b00011111

# get type
def asn1_get_type(der, (ixs,ixf,ixl)):
	return der[ixs]

# get length
def asn1_get_length(der, (ixs,ixf,ixl)):
	f = asn1_read_length(der, ixs)
	return f[2]-f[1]+1

## /end ADDED BY TeskaLabs ##
